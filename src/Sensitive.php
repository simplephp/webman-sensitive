...
class Sensitive
{
    ...
    {
        $config = config('plugin.simplephp.webman-sensitive.app', []);
        ...
        if ($mode == 'array') {
            ...
            $this->setFile(
                is_file($mode) ? $mode : config_path('plugin') . DIRECTORY_SEPARATOR . 'simplephp' . DIRECTORY_SEPARATOR . 'webman-sensitive' . DIRECTORY_SEPARATOR . 'SensitiveWord.txt'
            );
        }
    }
     ...
     * @throws SensitiveException
     */
    public function replace(string $content, string $replaceChar = '', bool $repeat = false, int $matchType = 1)
    {
        if (empty($content)) {
     ...
     * @throws SensitiveException
     */
    public function mark(string $content, string $tag = '', int $matchType = 1)
    {
        if (empty($content)) {
            throw new SensitiveException('Please fill in the content of the test', 1);
        }
        ...
        $badWordList = $this->badWordList ?: $this->get($content, $matchType);
     ...
     * @throws SensitiveException
     */
    public function get(string $content, int $matchType = 1, int $wordNum = 0)
    {
        $this->contentLength = sensitive_mb_strlen($content, 'utf-8');
        $badWordList = [];
     ...
     * @throws SensitiveException
     */
    public function add($words): Sensitive
    {
        if (!$this->wordTree) {
            throw new SensitiveException('Please initialize Sensitive first', 6);
        }

        if (is_string($words) && (false !== strpos($words, '|'))) {
            $words = explode('|', $words);
        }
     ...
     * @throws SensitiveException
     */
    public function remove($words, bool $once = false): Sensitive
    {
        if (!$this->wordTree) {
            throw new SensitiveException('Please initialize Sensitive first', 6);
        }
    ...
    {
        $contentLength = sensitive_mb_strlen($word, 'utf-8');
     ...
     * @throws SensitiveException
     */
    public function custom($custom): Sensitive
    {
        if (empty($custom)) {
            throw new SensitiveException('Custom sensitive words cannot be empty', 7);
        }
     ...
     * @throws SensitiveException
     */
    public function setFile(string $file): Sensitive
    {
        if (!is_file($file)) {
     ...
     * @throws SensitiveException
     */
    public function setTree(array $sensitiveWords = null): Sensitive
    {
     ...
     * @return $this
     */
    public function interferenceFactor(array $interferenceFactors): Sensitive
    {
     ...
     * @return string
     */
    public function ltrimInterferenceFactorBadWord(string $word): string
    {
        $characters = '';
     ...
     * @throws SensitiveException
     */
    public function addWordToFile($data, bool $append = true)
    {
        $mode = config('plugin.simplephp.webman-sensitive.app.mode', 'file');

        if ($mode == 'array') {
            throw new SensitiveException('Array mode cannot be added', 8);
        }

        $file = is_file($mode) ? $mode : config_path('plugin') . DIRECTORY_SEPARATOR . 'simplephp' . DIRECTORY_SEPARATOR . 'webman-sensitive' . DIRECTORY_SEPARATOR . 'SensitiveWord.txt';
     ...
     * @throws SensitiveException
     */
    protected function dfaBadWordConversChars($word, $char): string
    {
        $str = '';
     ...
     * @return bool
     */
    protected function checkInterferenceFactor(string $word): bool
    {
     ...
     * @return string
     */
    protected static function getSensitiveWordPath($path, $update = false): string
    {
        static $pathMaps = [];
